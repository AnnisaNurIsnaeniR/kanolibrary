...
func UpdateBook(w http.ResponseWriter, r *http.Request) {
	...
	if r.Method == "POST" {
		...
		result, err := crud.UpdateOne("books", bson.M{"_id": objId}, bson.M{"$set" :bson.M{"title": book.Title, "author": book.Author, "publisher": book.Publisher, "synopsis": book.Synopsis}})
		if err != nil {
			http.Error(w, err.Error(), http.StatusInternalServerError)
			return
		}
